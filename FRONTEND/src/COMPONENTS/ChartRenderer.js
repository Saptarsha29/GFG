...
const ChartRenderer = ({ chart, onBarClick }) => {
  ...
  // 🔥 FIX: Mathematical logic for Single Pillar vs Multiple Pillars
  const dataMin = Math.min(...formattedData.map(d => d.value));
  const dataMax = Math.max(...formattedData.map(d => d.value));
  let yDomain;

  if (dataMax === dataMin) {
    // Agar sirf ek hi value hai (North America), toh domain ko 0 se start karo
    yDomain = [0, dataMax === 0 ? 100 : dataMax * 1.2];
  } else {
    // Agar zyada values hain, toh zoom effect lagao
    const padding = (dataMax - dataMin) * 0.1; 
    yDomain = dataMin > 0 ? [Math.max(0, dataMin - padding), dataMax + padding] : ['auto', 'auto'];
  }
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={formattedData}>
              ...
              <YAxis 
                ...
                domain={yDomain} // Fixed domain apply ho raha hai
              />
              <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(255, 255, 255, 0.05)' }} />
              <Bar dataKey="value" radius={[8, 8, 0, 0]}>
                {formattedData.map((entry, index) => (
